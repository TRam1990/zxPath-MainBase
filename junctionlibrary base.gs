...
public bool Any_Lock(int id1, int dir1, bool poshorstn,int i,int num, bool poeznoi, bool remove)
	...
	if(!MO0.isclass(Junction))
		...
		if(i==0)
			...
			if(poeznoi)
				{
				while(!MO1.isclass(Junction) and !(MO1.isclass(Signal) and ( MO1.GetProperties().GetNamedTagAsInt("GetSignalType()",1) & (2+4+8) )  ))
					{
					MO1=GSTS.SearchNext();
...
// разборка маршрута по освобождению стрелки
			...
			if(p_element_n>=0 and (cast<PathClass>(PathLib.DBSE[p_element_n].Object)).mode>=0)
				...
				if(poeznoi and (pos == 2 or pos == 3) and msg.src.isclass(Train) )				// последняя стрелка поездного маршрута разбирается светофором
					{
					return;
					}
